...
public class ListNominas extends JDialog {
	...
	private Nomina aux = null;

	public ListNominas() {
			setTitle("Lista de Nominas");
			...
			contentPanel.setLayout(new BorderLayout(0, 0));
			{
				...
				String[] headers = {"Estado","ID","Nombre","Cédula","Monto a pagar","Fecha"};
				...
				table.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						if(seleccion != -1) {
							...
							aux = Altice.getInstance().buscarNomina((String)table.getValueAt(seleccion, 1));
						}
					}
				});
				...
				getContentPane().add(buttonPane, BorderLayout.SOUTH);
				{
					...
					btnMod.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							Altice.getInstance().getMisNominas().remove(aux);
							llenarTabla();
						}
					});
		...
		public static void llenarTabla() {
			...
			for (Nomina nom : Altice.getInstance().getMisNominas()) {
					rows[0] = nom.getEstado();
					rows[1] = nom.getId();
					rows[2] = nom.getEmp().getNombre();
					rows[3] = nom.getEmp().getCedula();
					rows[4] = String.format("%.1f", nom.getEmp().sueldoTotal());
					rows[5] = nom.getFechaDeEmision().getDate() + "/" +(nom.getFechaDeEmision().getMonth()+1) + "/" + (nom.getFechaDeEmision().getYear()+1900);
				modelo.addRow(rows);
			}
		}
